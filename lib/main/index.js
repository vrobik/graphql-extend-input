"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = void 0;

var _utilities = require("graphql/utilities");

var _os = require("os");

var _eol = require("eol");

const INDENT_SPACE = '  ';
const INPUT_NAME_LOOKUP_PATTERN = /([\W]|^)(input|enum)\s+(\w+)[\s]{0,}\{?/;
const EXTEND_PATTERN = /((\w+)\s+extend|^[\s+]{0,}extend)\s+(input|enum)\s+(\w+)[\s+]?(([\s]{0,},[\s]{0,}(\w+)[\s]{0,})+){0,}\{?/;
const INNER_PROP_PATTERN = /(\w{1,}:\s{0,}[\w\[\]!]{1,}!{0,}){0,}\}/;
const ONE_LINE_EXTEND = /[\w\s]+\{(\s){0,}\}/;

const getInnerProps = (lines, startIndex) => {
  const linesToAdd = [];
  if (!lines) return null;
  if (lines[startIndex] && lines[startIndex].match(ONE_LINE_EXTEND)) return [];

  for (let i = startIndex + 1; i < lines.length; i++) {
    const line = lines[i].match(INNER_PROP_PATTERN);

    if (line) {
      if (line[1]) linesToAdd.push(line[1]);
      return linesToAdd;
    }

    linesToAdd.push(lines[i]);
  }

  return null;
};

const isInnerLine = line => {
  if (typeof line === 'string') return line.match(/[\{\}]/) === null;
  return null;
};

const cleanLine = (line, inner = true) => {
  if (typeof line === 'string') return line.replace(/^\s+/g, inner ? INDENT_SPACE : '');
};

const getAdditionalMatches = group => {
  if (!group) return;
  const additional = [];
  group.split(',').forEach(item => {
    const add = item.replace(/ /g, '');
    if (add) additional.push(add);
  });
  return additional;
};

const extendInputFromText = (input, extend) => {
  const linesExtend = extend.split(_os.EOL);
  const additionalSchema = [];
  let changedLinesText = '';

  for (let i = 0; i < linesExtend.length; i++) {
    const lineExtend = linesExtend[i];
    const matchs = lineExtend.match(EXTEND_PATTERN);

    if (!matchs || !matchs[4]) {
      additionalSchema.push(lineExtend);
      continue;
    }

    const inputName = matchs[4];
    const inputOrEnum = matchs[3];
    const newInputName = matchs[2];
    if (!inputName) continue;
    const linesToAdd = getInnerProps(linesExtend, i);
    i += linesToAdd.length + 1;
    const linesInput = input.split(_os.EOL);
    const additionalMatches = getAdditionalMatches(matchs[5]);
    if (additionalMatches && additionalMatches.length) for (const addInput of additionalMatches) for (let z = 0; z < linesInput.length; z++) {
      const matchs = linesInput[z].match(INPUT_NAME_LOOKUP_PATTERN);
      if (matchs && matchs[3] === addInput && matchs[2] === inputOrEnum) linesToAdd.push(...getInnerProps(linesInput, z));
    }

    for (let ii = 0; ii < linesInput.length; ii++) {
      const lineInput = linesInput[ii];
      const matchs = lineInput.match(INPUT_NAME_LOOKUP_PATTERN);
      if (!matchs || matchs[2] !== inputOrEnum || matchs[3] !== inputName) continue;
      linesInput.splice(ii + 1, 0, ...linesToAdd);

      if (!newInputName) {
        input = linesInput.join(_os.EOL);
        const changed = [lineInput, ...getInnerProps(linesInput, ii), '}' + _os.EOL];
        changedLinesText += changed.join(_os.EOL);
      } else {
        const changed = [lineInput.replace(inputName, newInputName), ...getInnerProps(linesInput, ii), '}' + _os.EOL];
        const textChanged = changed.join(_os.EOL);
        input += textChanged;
        changedLinesText += textChanged;
      }

      break;
    }
  }

  const additionalSchemaText = additionalSchema.length ? additionalSchema.join(_os.EOL) : '';
  const finalInput = input + additionalSchemaText;
  return {
    schema: finalInput,
    partial: changedLinesText + additionalSchemaText
  };
};

const formatSchema = schemaText => {
  if (typeof schemaText !== 'string') return '';
  const text = [];
  schemaText.split(_os.EOL).forEach(line => {
    if (line.replace(/ /g, '') === '') return;
    const inner = isInnerLine(line);
    text.push(cleanLine(line, inner));
  });
  return text.join(_os.EOL);
};

const gqlExtI = (schema, extendGraphqlText, returnCompleteSchema = false) => {
  const schemaText = typeof schema === 'string' ? schema : (0, _utilities.printSchema)(schema);
  const newSchema = extendInputFromText((0, _eol.auto)(schemaText), (0, _eol.auto)(extendGraphqlText));
  if (returnCompleteSchema) return formatSchema(newSchema.schema);
  return formatSchema(newSchema.partial);
};

var _default = gqlExtI;
exports.default = _default;
module.exports = exports["default"];